...
export class GetRealEstateTransactionUseCase {
  ...
  /**
   * 모든 **아파트** 실거래가 데이터를 자동 페이지네이션으로 수집
   * @param lawdCd 지역코드 (법정동코드 5자리)
   * @param dealYmd 계약년월 (YYYYMM 형식)
   * @param options 추가 옵션
   * @returns 통합된 응답 데이터
   */
  async getAllTransactionsByLocationAndDate(
    lawdCd: string,
    dealYmd: string,
    options: {
      maxItems?: number; // 최대 수집할 아이템 수 (선택적)
      batchSize?: number; // 한 번에 가져올 아이템 수 (기본값: 1000)
      serviceKey?: string;
    } = {}
    ...
    try {
      ...
      console.log(`🔍 ${lawdCd} 지역 ${dealYmd} 아파트 전체 실거래가 수집 시작...`);
      
      while (allItems.length < maxItems) {
        console.log(`📄 아파트 페이지 ${pageNo} 수집 중... (현재 ${allItems.length}개 수집됨)`);
        ...
        if (!firstResponse) {
          ...
          console.log(`📊 아파트 전체 데이터 수: ${totalCount}개`);
        }
        
        if (!response.body.items.item || response.body.items.item.length === 0) {
          console.log(`✅ 아파트 더 이상 데이터가 없습니다.`);
          break; // 더 이상 데이터가 없음
        }
        ...
        if (allItems.length >= totalCount) {
          console.log(`✅ 아파트 전체 데이터 수집 완료: ${allItems.length}개`);
          break;
        }
        ...
        // 무한 루프 방지
        if (pageNo > 1000) {
          console.warn(`⚠️ 아파트 페이지 수가 1000을 초과하여 중단합니다.`);
          break;
        }
      }
      ...
      console.error('아파트 전체 실거래가 수집 실패:', error);
      throw error;
    }
  }

  /**
   * 단독/다가구 모든 실거래가 데이터를 자동 페이지네이션으로 수집
   */
  async getAllDetachedHouseTransactionsByLocationAndDate(
    lawdCd: string,
    dealYmd: string,
    options: {
      maxItems?: number;
    ...
    try {
      ...
      console.log(`🔍 ${lawdCd} 지역 ${dealYmd} 단독/다가구 전체 실거래가 수집 시작...`);
      
      while (allItems.length < maxItems) {
        console.log(`📄 단독/다가구 페이지 ${pageNo} 수집 중... (현재 ${allItems.length}개 수집됨)`);
        ...
        if (!firstResponse) {
          firstResponse = response;
          totalCount = parseInt(response.body.totalCount);
          console.log(`📊 단독/다가구 전체 데이터 수: ${totalCount}개`);
        }
        
        if (!response.body.items.item || response.body.items.item.length === 0) {
          console.log(`✅ 단독/다가구 더 이상 데이터가 없습니다.`);
          break;
        }
        ...
        // 전체 데이터를 다 가져왔는지 확인
        if (allItems.length >= totalCount) {
          console.log(`✅ 단독/다가구 전체 데이터 수집 완료: ${allItems.length}개`);
          break;
        }
        ...
        // 무한 루프 방지
        if (pageNo > 1000) {
          console.warn(`⚠️ 단독/다가구 페이지 수가 1000을 초과하여 중단합니다.`);
          break;
        }
      }
      ...
      console.error('단독/다가구 전체 실거래가 수집 실패:', error);
      throw error;
    }
  }

  /**
   * 오피스텔 모든 실거래가 데이터를 자동 페이지네이션으로 수집
   */
  async getAllOfficetelTransactionsByLocationAndDate(
    lawdCd: string,
    dealYmd: string,
    options: {
      maxItems?: number;
    ...
    try {
      ...
      console.log(`🔍 ${lawdCd} 지역 ${dealYmd} 오피스텔 전체 실거래가 수집 시작...`);
      
      while (allItems.length < maxItems) {
        console.log(`📄 오피스텔 페이지 ${pageNo} 수집 중... (현재 ${allItems.length}개 수집됨)`);
        ...
        if (!firstResponse) {
          firstResponse = response;
          totalCount = parseInt(response.body.totalCount);
          console.log(`📊 오피스텔 전체 데이터 수: ${totalCount}개`);
        }
        
        if (!response.body.items.item || response.body.items.item.length === 0) {
          console.log(`✅ 오피스텔 더 이상 데이터가 없습니다.`);
          break;
        }
        ...
        // 전체 데이터를 다 가져왔는지 확인
        if (allItems.length >= totalCount) {
          console.log(`✅ 오피스텔 전체 데이터 수집 완료: ${allItems.length}개`);
          break;
        }
        ...
        // 무한 루프 방지
        if (pageNo > 1000) {
          console.warn(`⚠️ 오피스텔 페이지 수가 1000을 초과하여 중단합니다.`);
          break;
        }
      }
      ...
      console.error('오피스텔 전체 실거래가 수집 실패:', error);
      throw error;
    }
  }

  /**
   * 연립다세대 모든 실거래가 데이터를 자동 페이지네이션으로 수집
   */
  async getAllRowHouseTransactionsByLocationAndDate(
    lawdCd: string,
    dealYmd: string,
    options: {
      maxItems?: number;
    ...
    try {
      ...
      console.log(`🔍 ${lawdCd} 지역 ${dealYmd} 연립다세대 전체 실거래가 수집 시작...`);
      
      while (allItems.length < maxItems) {
        console.log(`📄 연립다세대 페이지 ${pageNo} 수집 중... (현재 ${allItems.length}개 수집됨)`);
        ...
        if (!firstResponse) {
          firstResponse = response;
          totalCount = parseInt(response.body.totalCount);
          console.log(`📊 연립다세대 전체 데이터 수: ${totalCount}개`);
        }
        
        if (!response.body.items.item || response.body.items.item.length === 0) {
          console.log(`✅ 연립다세대 더 이상 데이터가 없습니다.`);
          break;
        }
        ...
        // 전체 데이터를 다 가져왔는지 확인
        if (allItems.length >= totalCount) {
          console.log(`✅ 연립다세대 전체 데이터 수집 완료: ${allItems.length}개`);
          break;
        }
        ...
        // 무한 루프 방지
        if (pageNo > 1000) {
          console.warn(`⚠️ 연립다세대 페이지 수가 1000을 초과하여 중단합니다.`);
          break;
        }
      }
      ...
      console.error('연립다세대 전체 실거래가 수집 실패:', error);
      throw error;
    }
  }
