...
/**
 * 연립다세대 매매 실거래가 조회 API
 * GET /api/transaction/row-house
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const response: GetRealEstateTransactionResponse = await useCase.getRowHouseTradeTransactions(requestData);
    ...
    console.error('연립다세대 매매 실거래가 조회 API 에러:', error);
