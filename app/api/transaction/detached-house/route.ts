...
/**
 * 단독/다가구 매매 실거래가 조회 API
 * GET /api/transaction/detached-house
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const response: GetRealEstateTransactionResponse = await useCase.getDetachedHouseTradeTransactions(requestData);
    ...
    console.error('단독/다가구 매매 실거래가 조회 API 에러:', error);
