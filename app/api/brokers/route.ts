  ...
  try {
    ...
    if (!brkrNm) {
      return NextResponse.json({ 
        success: false, 
        error: 'brkrNm parameter is required' 
      }, { status: 400 });
    }
    ...
    if (!userAddressNickname) {
      return NextResponse.json({ 
        success: false, 
        error: 'userAddressNickname parameter is required for BrokerCopy storage' 
      }, { status: 400 });
    }
    ...
    try {
      ...
      if (!brokerCopyResult.success) {
        console.warn('⚠️ BrokerCopy 저장 실패:', brokerCopyResult.error);
      }
    } catch (error) {
      console.warn('⚠️ BrokerCopy 저장 중 오류:', error);
