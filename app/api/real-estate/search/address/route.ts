...
export async function POST(request: NextRequest) {
  try {
    ...
    // UseCase 호출
    let response: GetRealEstateResponse;
    ...
    if (isCodefSuccess) {
      ...
      try {
        ...
        // 기존 데이터 확인
        const existing = await dbUseCase.findRealEstateCopyByUserAddressId(
          body.userAddressId
        );
        isUpdated = !!existing;
      ...
      return NextResponse.json(
        {
          ...
          resAddrList: response.data?.extraInfo?.resAddrList || [],
          data: response,
