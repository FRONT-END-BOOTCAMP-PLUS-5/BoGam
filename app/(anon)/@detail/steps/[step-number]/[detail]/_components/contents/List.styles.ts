const styles = {
  ...
  accordionContainer:
    'w-full border border-brand-light-gray rounded-lg overflow-hidden mb-2',
  accordionButton:
    'w-full px-4 py-3 text-left hover:bg-brand-light-gray hover:bg-opacity-80 active:bg-brand-white focus:bg-brand-white transition-colors duration-100 flex justify-between items-center border-b border-brand-light-gray',
  accordionText: 'text-brand-black text-sm font-medium ml-4',
  accordionArrow:
    'transform transition-transform duration-100 text-brand-dark-gray',
  accordionContent: 'px-4 py-3 bg-brand-white',
  accordionContentText:
    'text-brand-dark-gray leading-relaxed whitespace-pre-line text-sm',
};
